...
//Due to synchronization issues, we might have to change
//the value of this variable to ensure we stay below 64 entries
...
void filesys_cache_remove(disk_sector_t sector)
{
    ...
    //printf("delelting in cache_remove, size %d\n",thread_current()->name,  hash_size(&cache_hash));
...
look_and_lock(disk_sector_t sector, struct cache_hash_entry **entry_ptr)
{
  ...
  while (in_eviction_list) {
	...
	//printf("%s looking for %d in eviction list\n", thread_current()->name, sector);
	if (f->sector==sector){
	  //printf("%s in eviction list\n", thread_current()->name);
...
setup_cache_entry(disk_sector_t sector)
{
  ...
  //if (hash_size(&cache_hash)>64) printf(" %s Hash size is %d\n",thread_current()->name,  hash_size(&cache_hash));
  ASSERT(hash_size(&cache_hash)<=(unsigned) MAX_ENTRIES); 
  
  
  //TODO: think about the next two lines and
  //determine if there are problems with it.
  //You can't change the while to an if
  //because someone can come in in the middle of cache_out and 
  //add an entry(because we release the lock)...
  // trust me, i saw it in syn write... call me
  // if you have a question. tom
  while (hash_size(&cache_hash)==(unsigned) MAX_ENTRIES){
     //printf("%s cache full, evicting \n",thread_current()->name); 
    cache_out(); 
  }
  ...
  entry->ref_count = 0;
  //printf("%s insert in setup_cache_hash_entry, size %d\n",thread_current()->name,  hash_size(&cache_hash));
...
void cache_out()
{
  ...
  //printf("%s Deleting in cache out, size %d\n",thread_current()->name,  hash_size(&cache_hash));
  ...
  evictee->sector = best_evict->sector;
  lock_acquire(&eviction_list_lock);
  // printf("%s adding sector %d to eviction list\n", thread_current()->name,evictee->sector); 
  ...
  //WARNING: you remove the printf and you get a deadlock
  //printf("here\n");
  while (best_evict->ref_count>0); //{
    //printf("in here\n");
  //}
  //printf("and here\n");
  ...
  //parent function assumes it never got released
  lock_acquire(&cache_hash_lock);
...
//change struct sector_data to unsigned char array
